...
const entryPoints = entry.includes('*')
  ? await glob(entries, { absolute: false, onlyFiles: true, absWorkingDir, root: absWorkingDir })
  : entries.map((p) => path.relative(absWorkingDir, path.resolve(absWorkingDir, p)));
