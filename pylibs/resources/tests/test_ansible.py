...
class TestPaths(unittest.TestCase):
    ...
    def test_initializes_home_ops_paths_correctly(self):
        ...
        self.assertEqual(home_ops_paths.AUTOMATION_HOME, f"{XdgPaths.XDG_DATA_HOME}/automation")
        self.assertEqual(home_ops_paths.PROOT, f"{home_ops_paths.AUTOMATION_HOME}/home-ops")
    ...
    def test_initializes_ansible_paths_correctly(self):
        ...
        self.assertEqual(ansible_paths.PBROOT, f"{HomeOpsPaths.PROOT}/ansible/playbooks")
        ...
        self.assertEqual(ansible_paths.COLLECTIONS_REQS, f"{ansible_paths.CHOME}/requirements.yml")
        self.assertEqual(ansible_paths.ROLES_REQS, f"{ansible_paths.RHOME}/requirements.yml")
        self.assertEqual(ansible_paths.ALL_KVM_VARS, f"{ansible_paths.GVHOME}/proxmox_all_kvm.yaml")
        self.assertEqual(ansible_paths.ALL_VARS, f"{ansible_paths.GVHOME}/all.yaml")
        self.assertEqual(ansible_paths.ALL_LXC_VARS, f"{ansible_paths.GVHOME}/proxmox_all_lxc.yaml")
        self.assertEqual(ansible_paths.CHEZMOI_DATA, f"{ansible_paths.GVHOME}/chezmoi_data.yaml")
        self.assertEqual(ansible_paths.STATIC_HOSTS, f"{ansible_paths.IHOME}/hosts.yaml")
        self.assertEqual(ansible_paths.STATIC_HOSTS_YAML, ansible_paths.STATIC_HOSTS)
        self.assertEqual(ansible_paths.STATIC_HOSTS_TOML, f"{ansible_paths.IHOME}/hosts.toml")
        self.assertEqual(ansible_paths.DYNAMIC_NMAP, f"{ansible_paths.IHOME}/nmap.yaml")
        self.assertEqual(ansible_paths.DYNAMIC_PROXMOX, f"{ansible_paths.IHOME}/proxmox.yaml")
        self.assertEqual(ansible_paths.DYNAMIC_LDAP, f"{ansible_paths.IHOME}/microsoft.ad.ldap.yaml")
        self.assertEqual(ansible_paths.DYNAMIC_SQLITE, f"{ansible_paths.IHOME}/sqlite.yaml")
        self.assertEqual(ansible_paths.DBS_SQLITE, f"{XdgPaths.XDG_STATE_HOME}/sqlite/dasbootstrap.db")
        self.assertEqual(ansible_paths.INVENTORY_ALL, f"{ansible_paths.IHOME}/inventory/")
